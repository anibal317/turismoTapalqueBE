...
export class CityPointOfInterestService {
  ...
  async create(createCityPointOfInterestDto: CreateCityPointOfInterestDto): Promise<CityPointOfInterest> {
    ...
    // Verifica si el typeId existe en la base de datos
    const type = await this.cityPointOfInterestRepository.query(
    ...
    // Verifica si el subtypeId existe en la base de datos
    const subtype = await this.cityPointOfInterestRepository.query(
      'SELECT id FROM subtype WHERE id = ?',
      [subtypeId]
    );
    if (!subtype.length) {
    ...
    // Verifica si el idUser existe en la base de datos
    const user = await this.cityPointOfInterestRepository.query(
